...
#if defined(NDEBUG)
# error "Daffodils cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb70fcab8362b8319a50bd33b64192746c0a9a4c40acf42aba5f0ec67c4a8e8df");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Daffodils: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Daffodils Signed Message:\n";
...
{
    // Daffodils: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 5 * 60; // Daffodils: 5 minutes
static const int64 nTargetSpacing = 5 * 60; // Daffodils: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Daffodils: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Daffodils: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb70fcab8362b8319a50bd33b64192746c0a9a4c40acf42aba5f0ec67c4a8e8df");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Daffodils is developed using blockchain technology.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x5b12ff3fcd2a428dea305e11fb01aeab44d89943ba2d47684b2c55675876b843"));
...
unsigned char pchMessageStart[4] = { 0x4b, 0xc4, 0xba, 0x6b }; // Daffodils: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DaffodilsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DaffodilsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DaffodilsMiner(CWallet *pwallet)
{
    printf("DaffodilsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("daffodils-miner");
    ...
    try { loop {
        ...
        printf("Running DaffodilsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DaffodilsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DaffodilsMiner, pwallet));
}
